'use client';
import { useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { getReports, getAvailableMonths, syncToReportSheet } from '@/lib/api';

const LS_SS_URL_KEY = 'exportSpreadsheetUrl';

type Status = 'idle' | 'loading' | 'success' | 'error';

function fmtMonth(m: string) {
  const [y, mo] = m.split('-');
  return `${y}年${parseInt(mo)}月`;
}

function extractSpreadsheetId(input: string): string | null {
  const m = input.match(/\/d\/([a-zA-Z0-9_-]{20,})/);
  if (m) return m[1];
  // IDそのもの（URLでない場合）
  if (/^[a-zA-Z0-9_-]{20,}$/.test(input.trim())) return input.trim();
  return null;
}

const EXPORT_TYPES = [
  { id: 'daily',   label: '行動量・日次データ',  desc: '各メンバーのタブ（訪問・対面・商談・獲得）を作成・更新' },
  { id: 'summary', label: '月次サマリー',          desc: 'メンバー別の月間集計行を追加・更新' },
];

export default function ExportPage() {
  const router = useRouter();
  const [months, setMonths]               = useState<string[]>([]);
  const [selectedMonth, setSelectedMonth] = useState('');
  const [ssUrl, setSsUrl]                 = useState('');
  const [exportTypes, setExportTypes]     = useState<string[]>(['daily', 'summary']);
  const [status, setStatus]               = useState<Status>('idle');
  const [errorMsg, setErrorMsg]           = useState('');
  const [lastSync, setLastSync]           = useState('');

  useEffect(() => {
    if (!localStorage.getItem('user')) { router.replace('/login'); return; }

    const saved = localStorage.getItem(LS_SS_URL_KEY) || '';
    setSsUrl(saved);

    const now = new Date();
    const cur = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    setSelectedMonth(cur);

    getReports().then(reports => {
      const ms = getAvailableMonths(reports);
      if (!ms.includes(cur)) ms.unshift(cur);
      setMonths(ms);
    });
  }, [router]);

  const spreadsheetId = extractSpreadsheetId(ssUrl);
  const canSubmit = !!spreadsheetId && !!selectedMonth && exportTypes.length > 0;

  const toggleType = (t: string) =>
    setExportTypes(prev => prev.includes(t) ? prev.filter(x => x !== t) : [...prev, t]);

  const handleExport = async () => {
    if (!canSubmit || !spreadsheetId) return;
    setStatus('loading');
    setErrorMsg('');
    localStorage.setItem(LS_SS_URL_KEY, ssUrl);

    const result = await syncToReportSheet({ month: selectedMonth, spreadsheetId, exportTypes });

    if (result.success) {
      setStatus('success');
      setLastSync(new Date().toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' }));
    } else {
      setStatus('error');
      setErrorMsg(result.error || '不明なエラー');
    }
  };

  return (
    <div className="min-h-screen bg-gray-100">
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3">
        <button onClick={() => router.back()} className="text-gray-500 text-sm font-medium">← 戻る</button>
        <h1 className="text-base font-bold text-gray-800">スプレッドシートに反映</h1>
      </div>

      <div className="max-w-lg mx-auto px-4 pt-5 pb-24 space-y-4 page-animate">

        {/* URL入力 */}
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">反映先スプレッドシート</p>
          <textarea
            value={ssUrl}
            onChange={e => { setSsUrl(e.target.value); setStatus('idle'); }}
            placeholder="GoogleスプレッドシートのURLを貼り付け"
            rows={2}
            className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-700 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
          />
          <p className={`text-xs mt-1.5 font-medium ${spreadsheetId ? 'text-green-600' : 'text-gray-400'}`}>
            {spreadsheetId ? `✅ ID確認済み` : 'URLを貼り付けると自動でIDを認識します'}
          </p>
        </div>

        {/* 対象月 */}
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">対象月</p>
          <select
            value={selectedMonth}
            onChange={e => { setSelectedMonth(e.target.value); setStatus('idle'); }}
            className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-800 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            {months.map(m => <option key={m} value={m}>{fmtMonth(m)}</option>)}
          </select>
        </div>

        {/* 反映するデータ */}
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">反映するデータ</p>
          <div className="space-y-2">
            {EXPORT_TYPES.map(opt => (
              <button
                key={opt.id}
                onClick={() => toggleType(opt.id)}
                className={`w-full flex items-start gap-3 p-3 rounded-xl border transition-all active:scale-[0.99] text-left ${
                  exportTypes.includes(opt.id) ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-gray-50'
                }`}
              >
                <span className={`mt-0.5 w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 ${
                  exportTypes.includes(opt.id) ? 'bg-blue-500 border-blue-500' : 'bg-white border-gray-300'
                }`}>
                  {exportTypes.includes(opt.id) && (
                    <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" viewBox="0 0 12 12">
                      <path d="M2 6l3 3 5-5"/>
                    </svg>
                  )}
                </span>
                <div>
                  <p className={`text-sm font-semibold ${exportTypes.includes(opt.id) ? 'text-blue-700' : 'text-gray-700'}`}>{opt.label}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{opt.desc}</p>
                </div>
              </button>
            ))}
          </div>
        </div>

        {/* 注意 */}
        <div className="bg-amber-50 rounded-2xl p-4 flex gap-3">
          <span className="flex-shrink-0">⚠️</span>
          <p className="text-xs text-amber-700 leading-relaxed">
            反映先スプレッドシートにGASのGoogleアカウントを<strong>編集者</strong>として追加してください。
          </p>
        </div>

        {/* ボタン */}
        <button
          onClick={handleExport}
          disabled={!canSubmit || status === 'loading'}
          className={`w-full py-4 rounded-2xl font-bold text-base transition-all active:scale-[0.98] ${
            !canSubmit || status === 'loading'
              ? 'bg-gray-200 text-gray-400'
              : 'bg-blue-600 text-white shadow-md shadow-blue-200 active:bg-blue-700'
          }`}
        >
          {status === 'loading' ? '反映中…' : `📤 ${selectedMonth ? fmtMonth(selectedMonth) : ''}のデータを反映する`}
        </button>

        {/* 結果 */}
        {status === 'success' && (
          <div className="bg-green-50 border border-green-200 rounded-2xl p-4 flex items-center gap-3">
            <span className="text-2xl">✅</span>
            <div>
              <p className="text-sm font-bold text-green-700">反映完了（{lastSync}）</p>
              <p className="text-xs text-green-600">{fmtMonth(selectedMonth)} のデータを書き込みました</p>
            </div>
          </div>
        )}
        {status === 'error' && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-center gap-3">
            <span className="text-2xl">❌</span>
            <div>
              <p className="text-sm font-bold text-red-700">エラー</p>
              <p className="text-xs text-red-600 break-all">{errorMsg}</p>
            </div>
          </div>
        )}
      </div>
    </div>
  );
}
